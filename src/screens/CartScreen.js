...
const CartScreen = ({ navigation }) => {
  ...
  // Available payment methods (only cash currently supported)
  const paymentMethods = [
    { id: 'cash', name: 'Tiền mặt', icon: 'cash', description: 'Thanh toán trực tiếp tại bệnh viện' },
  ];

  // Upcoming payment methods (for display purposes)
  const upcomingPaymentMethods = [
    { id: 'vnpay', name: 'VNPAY', icon: 'card', description: 'Đang ký hợp đồng với ngân hàng', disabled: true },
    { id: 'momo', name: 'MoMo', icon: 'wallet', description: 'Đang hoàn thiện thủ tục pháp lý', disabled: true },
  ];
          ...
          onPress: async () => {
            ...
            try {
              ...
              if (selectedPayment.id === 'cash') {
                // Thanh toán tiền mặt - tạo payment record, xóa giỏ hàng và về trang chủ
                try {
                  console.log('🔗 Calling payment API:', `${API_BASE_URL}/api/payments/cash`);
                  console.log('📦 Payment request body:', { orderId: newOrder._id || newOrder.id });
                  ...
                  if (response.ok) {
                    ...
                    Alert.alert(
                      ...
                      'Cảm ơn bạn đã đặt hàng! Đơn hàng của bạn sẽ được xác nhận trong thời gian sớm nhất.',
                      [
                        {
                          text: 'OK',
                          onPress: () => {
                    ...
                    throw new Error(paymentResult.message || 'Lỗi tạo thanh toán');
                  }
                } catch (paymentError) {
                  console.error('Lỗi thanh toán tiền mặt:', paymentError);

                  // Xử lý lỗi network request failed
                  if (paymentError.message && paymentError.message.includes('Network request failed')) {
                    Alert.alert(
                      'Lỗi kết nối',
                      'Không thể kết nối đến máy chủ thanh toán. Vui lòng kiểm tra kết nối internet và thử lại.'
                    );
                  } else {
                    Alert.alert('Lỗi thanh toán', `Không thể tạo thanh toán tiền mặt: ${paymentError.message || 'Lỗi không xác định'}`);
                  }
                }
              } else if (selectedPayment.id === 'vnpay') {
                // Thanh toán VNPay - tạo payment URL và mở browser
                try {
                  console.log('🔗 Calling VNPay API:', `${API_BASE_URL}/api/payments/vnpay`);
                  const response = await fetch(`${API_BASE_URL}/api/payments/vnpay`, {
                    method: 'POST',
                    headers: {
                      'Content-Type': 'application/json',
                      'Authorization': `Bearer ${userToken}`
                  ...
                  if (response.ok && paymentResult.paymentUrl) {
                    console.log("VNPay URL created:", paymentResult.paymentUrl);
                    ...
                    // Điều hướng đến màn hình loading/waiting
                    ...
                    throw new Error(paymentResult.message || 'Lỗi tạo URL thanh toán VNPay');
                  }
                } catch (paymentError) {
                  ...
                  if (paymentError.message && paymentError.message.includes('Network request failed')) {
                    ...
                    Alert.alert('Lỗi thanh toán', `Không thể tạo thanh toán VNPay: ${paymentError.message || 'Lỗi không xác định'}`);
                  }
                }
              }
  ...
  const getServiceIcon = (service) => {
    // Nếu là số (service ID), trả về icon mặc định
    if (!service || typeof service === 'number' || /^\d+$/.test(service)) {
      return 'medkit';
    }

    // Nếu là service type string
    switch(service) {
          ...
          <>
            ...
            {appointmentInfo.patient.healthInsuranceId && (
              <Text style={styles.appointmentInfo}>
                ...
                {maskSensitiveData(appointmentInfo.patient.healthInsuranceId, 3)}
              </Text>
            )}
          </>
        )}
        ...
        {appointmentInfo.additionalServices && appointmentInfo.additionalServices.length > 0 && (
          <View style={styles.additionalServicesContainer}>
            ...
            {appointmentInfo.additionalServices.map((service, index) => (
              <Text key={`additional-${index}-${service.name}`} style={styles.additionalServiceItem}>
                • {service.name}
              </Text>
            ))}
          </View>
        ...
        <>
          <ScrollView style={styles.content}>
            ...
            <FlatList
              ...
              keyExtractor={(item, index) => `cart-item-${item.service}-${index}-${item.name || ''}`}
              scrollEnabled={false}
            />
              ...
              renderItem={({ item }) => (
                ...
                >
                  ...
                  <View style={styles.paymentOptionTextContainer}>
                    <Text style={[styles.paymentOptionText, item.disabled && styles.disabledText]}>{item.name}</Text>
                    {item.description && (
                      <Text style={[styles.paymentOptionDescription, item.disabled && styles.disabledText]}>
                        {item.description}
                      </Text>
                    )}
                    {item.disabled && (
