...
const handleRegister = async () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Send to backend for social registration
      const response = await fetch(`${API_BASE_URL}/api/auth/social-register`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Social registration failed');
      }
      ...
      Alert.alert('Thành công', 'Đăng ký với Google thành công!', [
        { text: 'OK', onPress: () => navigation.goBack() }
      ]);

    } catch (error) {
      console.error('❌ Google registration error:', error);
      Alert.alert('Lỗi đăng ký Google', error.message);
    } finally {
      setLoading(false);
    }
  };

  // Handle Apple Sign-In for Registration
  const handleAppleSignIn = async () => {
    ...
    try {
      ...
      Alert.alert('Thành công', 'Đăng ký với Apple thành công!', [
        { text: 'OK', onPress: () => navigation.goBack() }
      ]);

    } catch (error) {
      console.error('❌ Apple registration error:', error);
      Alert.alert('Lỗi đăng ký Apple', error.message);
    } finally {
      setLoading(false);
    }
  };
