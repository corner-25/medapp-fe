...
import { validateCCCD, validateBHYT, formatCCCD, formatBHYT } from '../utils/sensitiveDataUtils';
...
const RelativeFormScreen = ({ navigation, route }) => {
  ...
  const handleSave = async () => {
    ...
    if (relative.nationalId && relative.nationalId.trim()) {
      const cccdValidation = validateCCCD(relative.nationalId);
    ...
    if (relative.healthInsuranceId && relative.healthInsuranceId.trim()) {
      const bhytValidation = validateBHYT(relative.healthInsuranceId);
        ...
        <ScrollView style={styles.formContainer}>
          ...
          <View style={styles.inputGroup}>
            ...
            <View style={styles.inputWrapper}>
              ...
              <TextInput
                ...
                onChangeText={(text) => {
                  const formattedText = formatBHYT(text);
                  handleChange('healthInsuranceId', formattedText);
                }}
                editable={!loading}
                maxLength={19}
          ...
          <View style={styles.inputGroup}>
            ...
            <View style={styles.inputWrapper}>
              ...
              <TextInput
                style={styles.input}
                placeholder="Nhập CMND/CCCD (12 số)"
                keyboardType="number-pad"
                value={relative.nationalId}
                onChangeText={(text) => {
                  const formattedText = formatCCCD(text);
                  handleChange('nationalId', formattedText);
                }}
                editable={!loading}
                maxLength={15}
              />
