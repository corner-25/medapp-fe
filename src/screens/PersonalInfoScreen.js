...
import { validateCCCD, validateBHYT, formatCCCD, formatBHYT, maskSensitiveData } from '../utils/sensitiveDataUtils';
...
const PersonalInfoScreen = ({ navigation, route }) => {
  ...
  const handleSave = async () => {
    ...
    // Validate CCCD if editing nationalId
    if (currentField.key === 'nationalId' && currentField.value) {
      const cccdValidation = validateCCCD(currentField.value);
      if (!cccdValidation.isValid) {
        Alert.alert('CCCD không hợp lệ', cccdValidation.errors.join('\n'));
        return;
      }
    }

    // Validate BHYT if editing healthInsuranceId
    if (currentField.key === 'healthInsuranceId' && currentField.value) {
      const bhytValidation = validateBHYT(currentField.value);
      if (!bhytValidation.isValid) {
        Alert.alert('Mã thẻ BHYT không hợp lệ', bhytValidation.errors.join('\n'));
        return;
      }
    }
    ...
    try {
      // Chuẩn bị giá trị để lưu trong state (giữ nguyên format có space cho display)
      let valueForState = currentField.value;
      let valueForAPI = currentField.value;

      // Nếu là CCCD hoặc BHYT, chuẩn bị 2 giá trị khác nhau
      if (currentField.key === 'nationalId' && valueForState) {
        valueForAPI = valueForState.replace(/\s/g, ''); // Clean for API
        // valueForState giữ nguyên format có space cho hiển thị
      }
      if (currentField.key === 'healthInsuranceId' && valueForState) {
        valueForAPI = valueForState.replace(/\s/g, '').toUpperCase(); // Clean for API
        // valueForState giữ nguyên format có space cho hiển thị
      }

      // Cập nhật dữ liệu người dùng trong state (sử dụng clean value)
      const updatedUserInfo = {
        ...userInfo,
        [currentField.key]: valueForAPI // Lưu clean value trong state
      };
      ...
      if (isLoggedIn) {
        ...
        // Sử dụng valueForAPI đã được chuẩn bị ở trên
        const updateData = { [currentField.key]: valueForAPI };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content}>
        ...
        <View style={styles.infoSection}>
          ...
          <InfoItem
            label="Mã thẻ BHYT"
            value={userInfo?.healthInsuranceId ? (editMode ? formatBHYT(userInfo.healthInsuranceId) : maskSensitiveData(userInfo.healthInsuranceId, 3)) : ''}
            editMode={editMode}
          ...
          <InfoItem
            label="CMND/CCCD"
            value={userInfo?.nationalId ? (editMode ? formatCCCD(userInfo.nationalId) : maskSensitiveData(userInfo.nationalId, 3)) : ''}
            editMode={editMode}
          ...
          <View style={styles.modalContent}>
            ...
            <TextInput
              ...
              onChangeText={(text) => {
                ...
                // Auto-format CCCD and BHYT as user types
                if (currentField.key === 'nationalId' && text.length <= 12) {
                  formattedText = formatCCCD(text);
                } else if (currentField.key === 'healthInsuranceId' && text.length <= 15) {
                  formattedText = formatBHYT(text);
                }
                setCurrentField({...currentField, value: formattedText});
              }}
              placeholder={`Nhập ${currentField.label.toLowerCase()}`}
              keyboardType={
                currentField.key === 'nationalId' ? 'numeric' :
                currentField.key === 'healthInsuranceId' ? 'default' :
              ...
              editable={!loading}
              maxLength={currentField.key === 'nationalId' ? 15 : currentField.key === 'healthInsuranceId' ? 19 : undefined}
            />
