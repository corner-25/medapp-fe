...
const OrderDetailScreen = ({ navigation, route }) => {
  ...
  // Lấy icon dịch vụ - hỗ trợ cả service ID và service type
  const getServiceIcon = (serviceId) => {
    // Nếu là số (service ID), trả về icon mặc định dựa trên tên dịch vụ
    if (!serviceId || typeof serviceId === 'number' || /^\d+$/.test(serviceId)) {
      return 'medkit';
    }
